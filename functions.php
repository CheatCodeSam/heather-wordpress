<?php
/**
 * Heather functions and definitions
 *
...
if ( ! function_exists( 'heather_setup' ) ) :
	/**
	...
	function heather_setup() {
		 ...
		 * If you're building a theme based on Heather, use a find and replace
		 * to change 'heather' to the name of your theme in all the template files.
		 */
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'heather' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'heather_custom_background_args',
				array(
 ...
 */
function heather_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'heather_content_width', 640 );
}
add_action( 'after_setup_theme', 'heather_content_width', 0 );
 ...
 */
function heather_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'heather' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'heather' ),
...
add_action( 'wp_enqueue_scripts', 'heather_scripts' );
